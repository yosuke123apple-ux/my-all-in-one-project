...
//
// Lang は「言語コード」の型
// "ja" または "en" のどっちかしか入れられないようにする
type Lang = "ja" | "en";

// 1タスク分のデータ構造（日本語と英語のセット）
type TaskItem = {
  ja: string; // 日本語の文章
  en: string; // 英語の文章
};
...
// 「今日のタスク」の保存形式
// date: 今日の日付キー（例: "2026-2-1"）
// text: 今日表示するタスク文章（ja/enどっちか）
// completed: 今日タスクが完了したかどうか
type DailyTaskData = {
  date: string;
  text: string;
  completed: boolean;
...
// -------------------------------------------------
// 引数が省略されたら現在言語で表示する
function getTodayDate(lang: Lang = getCurrentLang()): string {
...
const monthYearEl = document.querySelector<HTMLSpanElement>(".month-year");
const prevBtn = document.querySelector<HTMLButtonElement>(".prev-month");
const nextBtn = document.querySelector<HTMLButtonElement>(".next-month");
...
const taskTypeSelect = document.querySelector<HTMLSelectElement>("#task-type"); // 今は未使用でもOK（将来拡張用）
const languageSelect = document.querySelector<HTMLSelectElement>("#language");
const progressEl = document.querySelector<HTMLDivElement>(".progress-bar");
const taskEl = document.querySelector<HTMLParagraphElement>(".task");
...
function getTodayTask(): DailyTaskData {
  ...
  let doneTasksJa: string[] = JSON.parse(localStorage.getItem("doneTasks") || "[]");
...
function setTodayTaskCompleted(completed: boolean): void {
...
let doneDays: number[] = JSON.parse(localStorage.getItem("doneDays") || "[]");
...
function applyLanguage(lang: Lang): void {
  // 設定モーダル内のテキストを取る
  const settingsTitleEl = document.querySelector<HTMLHeadingElement>(".settings-content h2");
  const taskTypeLabelEl = document.querySelector<HTMLLabelElement>('label[for="task-type"]');
  const languageLabelEl = document.querySelector<HTMLLabelElement>('label[for="language"]');
  const closeBtnEl = document.querySelector<HTMLButtonElement>(".close-settings");

  // i18n から言語に合う文字列を入れる
  if (settingsTitleEl) settingsTitleEl.textContent = i18n[lang].settingsTitle;
  if (taskTypeLabelEl) taskTypeLabelEl.textContent = i18n[lang].taskTypeLabel;
  if (languageLabelEl) languageLabelEl.textContent = i18n[lang].languageLabel;
  if (closeBtnEl) closeBtnEl.textContent = i18n[lang].close;

  // 今日の日付表示も言語に合わせる
  if (dateEl) dateEl.textContent = getTodayDate(lang);
...
function renderCalendar(date: Date = currentDate) {
...
function disableCompleteButton(): void {
...
function renderTodayTask(): void {
  ...
  // もし完了済みならボタンも無効化
  if (t.completed) disableCompleteButton();
}
...
// ✅ 今日のタスクを通知で出す
// -------------------------------------------------
function notifyTodayTask(taskText?: string) {
  // 通知許可がないなら何もしない
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  times.forEach((t, i) => {
    const el = document.getElementById(`notify-${i + 1}`) as HTMLInputElement;
    if (el) el.value = t;
  });
  ...
  languageSelect?.addEventListener("change", () => {
    ...
    const lang = (languageSelect.value as Lang) || "ja";
    ...
    // 日本語でも英語でも一致するようにしてる
    const taskItem = dailyTasks.find(
      (t) => t.ja === todayTask.text || t.en === todayTask.text
    );
...
function animateDay(dayNumber: number, date: Date) {
